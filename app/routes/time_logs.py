...
def list_time_logs():
    ...
    if request.headers.get("Accept", "").startswith("application/json") or request.args.get("format") == "json":
        return jsonify([{**l, "date": l["date"].strftime("%Y-%m-%d")} for l in all_logs])
    return render_template("time_logs/list.html", logs=all_logs, workers=workers, categories=TimeLog.DEFAULT_CATEGORIES)
...
def get_worker_logs(worker_id):
    ...
    return jsonify({
        ...
        "logs": [{"hours": l.hours, "categories": l.categories, "description": l.description, "date": l.date.strftime("%Y-%m-%d")} for l in logs]
    })
